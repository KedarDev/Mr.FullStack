...
const Resume = () => {
  ...
  return (
    <section
      id="resume"
      className="mobile-s:mt-[-400px] mt-[-35%] flex w-full p-2 xl:flex-row flex-col justify-center min-h-screen gap-10 max-container "
    >
      ...
      <div className="relative flex-1 justify-center items-center min-h-screen bg-hero bg-center">
        ...
        />
        <div className=" mobile-s:w-[120px] mobile-s:ml-[90px] mobile-s:mt-[-185px] relative mt-[-78%] flex-1 flex justify-center items-center xl:min-h-screen max-xl:py-40 bg-primary bg-cover bg-center">
          <img
            src={bigResumeImg}
            alt="resume collection"
            width={430}
            height={200}
            className=" mobile-s:w-[150px] object-contain relative z-10"
          />
        </div>
        <div className=" mobile-s:absoulte  flex sm:gap-6 gap-4 absoulte bottom-[5%]  mt-[100px] justify-center items-center content-center   max-sm:px-6 ">
          {resumes.map((resume) => (
            <div className="mobile-s:w-[20%]" key={resume}>
              <ResumeCard
                imgURL={resume}
                changeBigResumeImage={(resume) => setBigResumeImg(resume)}
                bigResumeImg={resume}
              />
              <h1 className=" flex mx-auto py-auto justify-center items-center  mobile-s:text-[10px]  text-[25px]">
                {resume.name}
              </h1>
            </div>
          ))}
