...
const Navbar = () => {
  ...
  return (
    <nav
    className={`${Styles.paddingX}  dark:tablet:w-[800px] mobile-s:w-screen wideScreen:w-screen dark:desktop:gri dark:desktop:bg-white dark:desktop:ml-[50px] dark:desktop:h-[66px]  desktop:grid desktop:grid-cols-4  dark:w-full dark:grid dark:grid-cols-4 laptop:w-full dark:desktop:grid-cols-5  dark:mobile:w-[680px] laptop:grid laptop:grid-cols-3  dark:p-1 dark:top-0 grid grid-cols-3 w-full  justify-center items-center mt-2 mx-auto px-5   `} 
    >
      <div className=" dark:bg-white dark:desktop:w-screen dark:laptop:w-[1024px] dark:laptop:h-[60px]  bg-white laptop:w-[1200px] w-screen h-fit dark:tablet:h-[60px] dark:tablet:w-[740px] ">
        {currentTheme === "dark" ? (
          <img
            src={darkflash}
            className=" dark:desktop:w-[62px] dark:laptop:ml-[] dark:tablet:ml-[20px] cursor-pointer dark:desktop:ml-[-60px] dark:tablet:w-[52px] dark:tablet:h-[59.5px] dark:bg-white mobile:w-[40px] mobile:h-[40px] w-[30px] h-[30px] top-3 left-0 object-cover flex justify-center items-center"
            alt=""
            onClick={toggleTheme}
          />
        ) : (
          <img
            src={lightFlash}
            className=" mobile-s:mt-[-16px] laptop:mt-[-28px] mobile:w-[40px] tablet:w-[52px] tablet:h-[59.5px] tablet:mt-[-34px] tablet:ml-[-2px] mobile:h-[40px]  w-[30px] mobile:left-0 absolute mobile:ml-[10px] mobile:mt-[-20px] h-[30px] mt-0  ml-[-20px] desktop:w-[40px] desktop:h-[40px]"
            alt=""
            onClick={toggleTheme}
          />
        )}
      ...
      <div className="w-full flex justify-between items-center max-w-7x1">
        ...
        >
          &nbsp;
          <span className=" dark:desktop:bg-white dark:mobile:ml-[160px] dark:mobile-s:ml-[60px] flex-shrink-0 justify-center dark:mobile-s:h-[] ">
            <img
              // onClick={}
              src={mflogo}
              alt="mflogo"
        ...
        <ul className="relative dark:desktop:bg-white wideScreen:ml-[540px] laptop:ml-[260px] z-0 list-none hidden lg:flex sm:hidden laptop:flex laptop-l:flex desktop:ml-[380px]  flex-row gap-5">
          {navLink.map((Link) => (
            <li
              key={Link.id}
              className={`${
        ...
        {/* Hamberger Menu */}
        <div className="  mobile-s:ml-[20px] z-0 lg:hidden laptop:hidden laptop-l:hidden desktop:hidden flex  justify-end items-center">
          <img
